...
class ClusterFig(BaseFig):
    ...
    def render(self, emb_3d:list[np.ndarray], clusters:list[Cluster], titles:list[str], ids:list[int]) -> str:
        ...
        fig.update_traces(
            ...
            hovertemplate='<b>%{hovertext}</b><br>'
                          'Offer ID: %{customdata[0]}<extra></extra>'
        )
        fig.update_layout(
            autosize=True,
